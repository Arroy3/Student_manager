...
function handleEdit()
{
    if (isset($_POST['btnSave'])) {
        ...
        if (empty($full_name)) {
            $_SESSION['error_update_user']['full_name'] = 'Enter full name of user, please';
        } elseif (!preg_match('/^[\p{L}\s]+$/u', $full_name)) {
            $_SESSION['error_update_user']['full_name'] = 'Invalid full name format, please enter alphabetic characters only';
        } else {
        ...
        if (empty($email)) {
            ...
            $_SESSION['error_update_user']['email'] = 'Invalid email format';
        } else {
            $_SESSION['error_update_user']['email'] = null;
        }
        if (empty($phone)) {
            $_SESSION['error_update_user']['phone'] = 'Please enter a phone number.';
        } elseif (!preg_match('/^\d{10}$/', $phone)) {
            $_SESSION['error_update_user']['phone'] = 'Please enter a 10-digit phone number.';
        } else {
        ...
        if (!empty($_FILES['avatar']['tmp_name'])) {
            $avatar = uploadFile($_FILES['avatar'], 'public/uploads/images/', ['image/png', 'image/jpg', 'image/jpeg', 'image/gif'], 5 * 1024 * 1024);
            if (empty($avatar)) {
...
function handleAdd()
{
    ...
    //     }
    //     if (empty($phone)) {
    //         $_SESSION['error_add_user']['phone'] = 'Please enter a phone number.';
    //     } elseif (!preg_match('/^\d{10}$/', $phone)) {
    //         $_SESSION['error_add_user']['phone'] = 'Please enter a 10-digit phone number.';
    //     } else {
    ...
    //     }
    //     if (empty($birthday)) {
    //         $_SESSION['error_add_user']['birthday'] = 'Enter name of birthday, please';
    //     } else {
...
function index()
{
    ...
    $users = getAllDataUsersByPage($keyword, $start, 10);
